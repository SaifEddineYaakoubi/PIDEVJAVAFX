...
import org.example.pidev.controllers.cultures.ConsulterCultureController;
...
public class DashboardController {
    ...
    private ClientService ahmedClientService;
    private VenteService ahmedVenteService;
    ...
    public void initialize() {
        ...
        try {
            ...
            ahmedClientService = new ClientService();
            ahmedVenteService = new VenteService();
    ...
    private void logout() {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            try {
                ...
                // Charger le CSS
                var cssUrl = getClass().getResource("/recoltes/smartfarmm.css");
                if (cssUrl != null) {
    ...
    private void setupNavigationButtons() {
        ...
        if (btnRecoltes != null) btnRecoltes.setOnAction(e -> { loadView("/recoltes/Recolte.fxml", "Récoltes"); buildRecolteStats(); showWeatherSection(false); loadAlerts(); applySidebarMode("recolte"); setActiveButton(btnRecoltes); });
        if (btnRendements != null) btnRendements.setOnAction(e -> { loadView("/recoltes/Rendement.fxml", "Rendements"); buildRendementStats(); showWeatherSection(false); loadAlerts(); applySidebarMode("rendement"); setActiveButton(btnRendements); });
        if (btnSoil != null) btnSoil.setOnAction(e -> { loadView("/recoltes/SoilDashboard.fxml", "Analyse du Sol"); buildDefaultStats(); showWeatherSection(false); applySidebarMode("default"); setActiveButton(btnSoil); });
        if (btnPredictions != null) btnPredictions.setOnAction(e -> { loadView("/recoltes/AIPredictionsWidget.fxml", "Prédictions IA"); buildDefaultStats(); showWeatherSection(false); applySidebarMode("default"); setActiveButton(btnPredictions); });
        if (btnArchive != null) btnArchive.setOnAction(e -> { loadView("/recoltes/Archive.fxml", "Archive"); buildDefaultStats(); showWeatherSection(false); applySidebarMode("default"); setActiveButton(btnArchive); });
        ...
        if (btnClients != null) btnClients.setOnAction(e -> { loadAhmedView("/ventes/ListeClient.fxml", "Clients"); buildClientStats(); showWeatherSection(false); applySidebarMode("default"); setActiveButton(btnClients); });
        if (btnVentes != null) btnVentes.setOnAction(e -> { loadAhmedView("/ventes/ListeVente.fxml", "Ventes"); buildVenteStats(); showWeatherSection(false); applySidebarMode("default"); setActiveButton(btnVentes); });
        if (btnRapports != null) btnRapports.setOnAction(e -> { loadAhmedView("/ventes/StatistiquesVente.fxml", "Rapports Ventes"); buildVenteStats(); showWeatherSection(false); applySidebarMode("default"); setActiveButton(btnRapports); });
        if (btnFidelite != null) btnFidelite.setOnAction(e -> { loadAhmedView("/ventes/FideliteStats.fxml", "Fidélité"); buildClientStats(); showWeatherSection(false); applySidebarMode("default"); setActiveButton(btnFidelite); });
        if (btnRelance != null) btnRelance.setOnAction(e -> { loadAhmedView("/ventes/RelanceClient.fxml", "Relance Clients"); buildClientStats(); showWeatherSection(false); applySidebarMode("default"); setActiveButton(btnRelance); });
        if (btnConversion != null) btnConversion.setOnAction(e -> { loadAhmedView("/ventes/ConversionStats.fxml", "Conversion"); buildClientStats(); showWeatherSection(false); applySidebarMode("default"); setActiveButton(btnConversion); });
    }
    ...
    private void loadDashboardView() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/recoltes/DashboardDefault.fxml"));
    ...
    private void loadCulturesView() {
        try {
            ...
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/cultures/consulterculture.fxml"));
            Parent view = loader.load();
    ...
    private void buildParcelleStats() {
        ...
        try {
            ...
            long actives = parcelles.stream().filter(p -> p.getEtat() != null && p.getEtat().toLowerCase().contains("activ")).count();
            long repos = parcelles.stream().filter(p -> p.getEtat() != null && p.getEtat().toLowerCase().contains("repos")).count();
            long exploitees = parcelles.stream().filter(p -> p.getEtat() != null && p.getEtat().toLowerCase().contains("exploit")).count();
    ...
    private void buildCultureStats() {
        ...
        try {
            ...
            long germination = cultures.stream().filter(c -> c.getEtatCroissance() != null && c.getEtatCroissance().toLowerCase().contains("germination")).count();
            long croissance = cultures.stream().filter(c -> c.getEtatCroissance() != null && c.getEtatCroissance().toLowerCase().contains("croissance")).count();
            long floraison = cultures.stream().filter(c -> c.getEtatCroissance() != null && c.getEtatCroissance().toLowerCase().contains("floraison")).count();
            long maturite = cultures.stream().filter(c -> c.getEtatCroissance() != null && c.getEtatCroissance().toLowerCase().contains("matur")).count();
            long recolte = cultures.stream().filter(c -> c.getEtatCroissance() != null && c.getEtatCroissance().toLowerCase().contains("récolté")).count();
            ...
            etatBox.getChildren().add(etatRow("🌻", null, "Mature", String.valueOf(maturite), "#fbbf24"));
            etatBox.getChildren().add(etatRow("✅", null, "Récolté", String.valueOf(recolte), "#a78bfa"));
            dynamicStatsContainer.getChildren().add(etatBox);
        } catch (Exception e) {
