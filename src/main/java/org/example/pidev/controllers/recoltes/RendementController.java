...
public class RendementController {
    ...
    private void handleAjouter() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/recoltes/AddRendement.fxml"));
    ...
    private void handleChercher() {
        ...
        for (Rendement r : rendementList) {
            if (String.valueOf(r.getProductivite()).contains(searchText) ||
                    String.valueOf(r.getSurfaceExploitee()).contains(searchText) ||
                    String.valueOf(r.getQuantiteTotale()).contains(searchText)) {
                filtered.add(r);
            }
        }
    ...
    private void handleModifier(Rendement rendement) {
        try {
            ...
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/recoltes/RendementDetail.fxml"));
            Parent root = loader.load();
    ...
    private void handleSupprimer(Rendement rendement) {
        ...
        alert.setContentText("Voulez-vous supprimer ce rendement ?");
